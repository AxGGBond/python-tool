...
"""
import os
from docx_civil_code_parser import DocxCivilCodeParser
...
def main():
    ...
    # 检查文件是否存在
    input_file = "local_file/1.中华人民共和国民法典.docx"
    if not os.path.exists(input_file):
        print(f"❌ 文件 {input_file} 不存在")
        print("请确保Word文档在当前目录中")
        return
    
    print(f"✅ 找到文件: {input_file}")
    
    # 创建解析器
    parser = DocxCivilCodeParser(api_key=api_key)
    
    # 预览文档
    print("\n📖 预览文档内容...")
    parser.preview_docx_content(input_file, max_articles=3)
